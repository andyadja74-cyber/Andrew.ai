...
// 🧠 INITIALISATION GEMINI IA (Nouveau SDK @google/genai)
const { GoogleGenAI } = require("@google/genai");
const apiKey = process.env.GEMINI_API_KEY;

if (!apiKey) {
  console.warn("⚠️ AVERTISSEMENT : La variable GEMINI_API_KEY n'est pas configurée dans l'environnement !");
}
...
async function startBot() {
  ...
  // 🔑 DEMANDE STABILISÉE DU CODE DE JUMELAGE
  if (!sock.authState.creds.registered) {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(`\n==================================`);
        console.log(`👉 CODE DE JUMELAGE : ${code?.match(/.{1,4}/g)?.join("-") || code}`);
        console.log(`==================================\n`);
      } catch (err) {
        console.error("❌ Erreur lors de la demande du Pairing Code :", err);
      }
    }, 6000); // Temporisation de 6 secondes pour laisser le socket se stabiliser
  }
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 🤖 FONCTIONNALITÉ GEMINI IA
      if (lowerText.startsWith('.ai') || lowerText.startsWith('.gemini')) {
        const prompt = cleanText.replace(/^(\.ai|\.gemini)\s*/i, '').trim();

        if (!prompt) {
          await envoyerAvecDelai(sock, remoteJid, { 
            text: "⚠️ Posez une question après la commande.\n\nExemple : `.ai Quelle est la capitale de la France ?`" 
          }, { quoted: msg }, msg);
          return;
        }
        ...
        try {
          ...
          await envoyerAvecDelai(sock, remoteJid, { 
            text: "❌ Une erreur s'est produite lors de la communication avec l'IA Gemini. Vérifiez votre clé d'API sur Render." 
          }, { quoted: msg }, msg);
        }
        return;
      }
      ...
      // 🎮 GESTION DE LA NAVIGATION DU LABYRINTHE (.lab)
...
🤖 *──────── 🧠 IA GEMINI ────────*
🔹 *.ai [Question]* ➔ *Poser une question à l'intelligence artificielle*
...
⚡ *━━━━━━━━━━━━━━━━━━━━━━━━━* ⚡`;
      ...
      // 🐾 ANIMAL DE COMPAGNIE (.toutou ET SOINS)
      if (lowerText === '.toutou' || lowerText === '.animal') {
        ...
        if (!pet.vivant) {
          await envoyerAvecDelai(sock, remoteJid, { 
            text: `💀 *VOTRE ANIMAL EST MORT DE NEGLIGENCES !*\n\n👉 Tapez **.toutou** à nouveau pour en adopter un autre.` 
          }, { quoted: msg }, msg);
          delete animauxJoueurs[senderJid];
          return;
        }

        const toutouText = `🐾 *CAPACITÉS & ÉTAT DE DE VOTRE ANIMAL* 🐾\n\n` +
          `🏷️ *Nom :* **${pet.nom}**\n` +
